...
def render_set_ply(args, scene, pipe, out_dir, tau, eval):
    ...
    for viewpoint in tqdm(cameras):
        ...
        if eval:
            ...
            print(f"tau: {tau}, PSNR: {psnr(image, gt_image).mean().double():.5f} SSIM: {ssim(image, gt_image).mean().double():.5f} LPIPS: {lpips(image, gt_image, net_type='vgg').mean().double():.5f}")
            print('image_name:', viewpoint.image_name)
            with open(os.path.join(render_path, "results.txt"), "a") as f:
                f.write(f"{viewpoint.image_name} PSNR: {psnr(image, gt_image).mean().double():.5f} SSIM: {ssim(image, gt_image).mean().double():.5f} LPIPS: {lpips(image, gt_image, net_type='vgg').mean().double():.5f}\n")
